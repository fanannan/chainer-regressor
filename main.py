...
def run(name, network, train_data, test_data, batch_size, num_epoch, test=False):
    plt.ion()
    fig = plt.figure()
    ...
    def draw_chart(xs):
        ...
        plt.draw()
    #
    network.run(train_data, test_data, batch_size, num_epoch, test=test, callback=draw_chart)
...
def execute(name,  train_data, test_data, batch_size, num_inputs, optimizer, num_epoch, gpu):
    if name == "MLP":
        import MLP
        num_units   = 30 # 中間層の数
        dropout_ratio = 0.5 # ドロップアウトの確率
        network = MLP.MLP(num_inputs, num_units, dropout_ratio, optimizer, gpu)
        run(name, network, train_data, test_data, batch_size, num_epoch, test=True)
    elif name == "dA":
        # 次元削減
        import dA
        ...
        base_network = run(name+'_pretune', base_network, train_data, test_data, batch_size, num_epoch, test=False)
        ...
        run(name+'_finetune', top_network, (base_x_train, train_data[1]), (base_x_test, test_data[1]), batch_size, num_epoch, test=True)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale', '-s', default=False, type=bool, help='Apply scaling')
    parser.add_argument('--normalize', '-nm', default=False, type=bool, help='Apply normalizing')
    args = parser.parse_args()
    # 確認進捗表示用
    logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
    # データ読み取り
    (train_data, test_data, info) = reader.read_dataset(train_size, normalize=False, scale=False)
    num_inputs = info["SHAPE_TRAIN_X"][1]
    # モデル実行
    optimizer = select_optimizer(args.optimizer)
    execute(args.network, train_data, test_data, batch_size, num_inputs, optimizer, num_epoch, args.gpu)
